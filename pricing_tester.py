...
def pricing_health():
    ...
    return {"ok": True, "message": "pricing tables ensured"}
...
def import_pricing_bundle(product_uuid: str, payload: dict, db: Session = Depends(get_db)):
    ...
    db.commit()
    return {"ok": True, "product_uuid": product_uuid, "groups": len(groups), "prices": inserted_prices}
...
<!doctype html>
<html>
<head><meta charset="utf-8" /><title>Pricing Tester</title></head>
<body style="font-family:Arial;padding:18px;max-width:980px;margin:0 auto;">
<h1>Pricing Tester</h1>
<p><b>Product UUID:</b> {product_uuid}</p>
<p>First hit <code>/pricing/health</code> to create tables, then import data.</p>
</body>
</html>
""")
